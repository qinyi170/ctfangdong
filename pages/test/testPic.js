...
Page({
  ...
  start: function (res) {
    ...
    if (res.touches.length == 1) {
      ...
      that.setData({
        oldclientX: res.touches[0].pageX,
        oldclientY: res.touches[0].pageY
      })
  ...
  move: function (res) {
    let that = this;
    if (res.touches.length == 1){
      templeft += res.touches[0].pageX*1 - that.data.oldclientX*1
      temptop += res.touches[0].pageY * 1 - that.data.oldclientY * 1
      that.setData({
        imgleft: templeft,
        imgtop: temptop,
        oldclientX: res.touches[0].pageX,
